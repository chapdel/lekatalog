...
<div class="@if($disabled) opacity-60 @endif">
    ...
    <div class="flex {{ !$label && $cornerHint ? 'justify-end' : 'justify-between' }} mb-1">
        @if ($label)
        <x-dynamic-component :component="WireUi::component('label')" :label="$label" :has-error="$hasError"
            :for="$id" />
        @endif

        @if ($cornerHint)
        <x-dynamic-component :component="WireUi::component('label')" :label="$cornerHint" :has-error="$hasError"
            :for="$id" />
        @endif
    </div>
    @endif
    ...
    <div class="relative rounded-md @unless($shadowless) shadow-sm @endunless">
        ...
        <div class="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none
                {{ $hasError ? 'text-negative-500' : 'text-gray-400' }}">
            @if ($icon)
    ...
    @if (!$hasError && $hint)
    <label @if ($id) for="{{ $id }}" @endif class="mt-2 text-sm text-gray-500 dark:text-gray-200">
        {{ $hint }}
    </label>
    @endif
